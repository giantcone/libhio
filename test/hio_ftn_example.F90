program hio_example
  ...
  integer(kind(MPI_ERROR))      :: retval_mpi, errc_mpi
  ...
  real(c_float),dimension(4096),target :: crap

  !.......................................................................................
  ! B E G I N   P R O G R A M
  !.......................................................................................

  ! I was going to do something fancy here, but for the sake of doing exactly the same as
  ! the libHIO example code in C, I'll keep it close to the same as possible.
  ...
  !  call data_init()
  ! initialize the data array in a simple fashion.
  crap = my_rank
...
contains
  ...
  subroutine initialize_hio()
    ...
    retval_hio = hio_init_mpi(my_hio_context, MPI_COMM_WORLD, "hio_example.input.dec", "#HIO", &
                    "hio_example_context")
    ...
    if (HIO_SUCCESS /= retval_hio) then
       ...
       call mpi_abort(MPI_COMM_WORLD, errc_mpi, retval_mpi)
    end if

  end subroutine initialize_hio
!......................................................................................................
  subroutine finalize_hio()

    retval_hio = hio_fini(my_hio_context)
    call mpi_comm_free(MPI_COMM_WORLD, retval_mpi)
    call mpi_finalize(retval_mpi)
...
!......................................................................................................
  subroutine checkpoint(wcount, stride, tstep)
    ...
    integer(kind(HIO_SCP_NOT_NOW)) :: hint_enum
    integer(kind(HIO_FLAG_CREAT))  :: flag_val
    integer(c_int)                 :: wrt_offset
    integer(c_size_t)              :: bytes_wrtn, sz_c_int, sz_c_size, sz_c_float
    ...
    sz_c_float = sz_c_int             ! The 'bit_size' intrinsic only works on Fortran integers.
    ...
    retval_hio = hio_element_close(element_ptr)
    ...
    retval_hio = hio_dataset_free(set_ptr)
    ...
    retval_hio = hio_dataset_alloc (my_hio_context, set_ptr, "restartShared", tstep, &
                   flag_val, HIO_SET_ELEMENT_SHARED)

    retval_hio = hio_dataset_open(set_ptr)

    ! Example of writing out a shared file
    retval_hio = hio_element_open(set_ptr, element_ptr, "shared", flag_val)
    
    wrt_offset = my_rank * sz_c_size

    retval_hio = hio_element_write_nb (element_ptr, req_ptr, wrt_offset, 0, wcount, 1, sz_c_size)

    wrt_offset = nranks*sz_c_size + wcount*sz_c_float*my_rank
    
    retval_hio = hio_element_write_strided_nb(element_ptr, req_ptr + 1_c_intptr_t, wrt_offset, 0, &
                   data_ptr, wcount, sz_c_float, stride)

    wrt_offset = nranks * (sz_c_size + wcount * sz_c_float) + my_rank * wcount * sz_c_int

    retval_hio = hio_element_write_strided_nb(element_ptr, req_ptr + 2_c_intptr_t, wrt_offset, 0, &
                   data_ptr + addr_offset, wcount, sz_c_int, stride)

    retval_hio = hio_request_wait(req_ptr, 3, bytes_xfrd)
